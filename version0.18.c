...
int pickBestReplace(int a[], int b[], int size) //both size are the same, a is always the modified page
{
	int i;
	int count_a = 0;
	int count_b = 0;
	...
	if(count_a - count_b > 3) //return a(modified)
	{
		return 1;
	}
	else if(count_a - count_b <= 3) // return b (non modified)
	{
		return 0;
	}
	else
...
void pickLowestDecBit2(struct pageQueue* q, int new_base, int new_limit, int num_of_pages,long int* totalWp, int clock)
{
	struct Page* curr;
	curr = q->front;

	//lowest at start is the front's dec_bit
	int lowest_dec_bit1 = 255; //for mod = 0
	int lowest_dec_bit2 = 255; //for mod = 1
	int lowest_arrival_time1 = 1000001; //for mod = 0
	int lowest_arrival_time2 = 1000001; //for mod = 1
	int cdec;
	int i,j;
	...
	//find what is the lowest_dec_bit for mod = 0
	for (i=1;i<num_of_pages;i++)
	{
		// printf("Base %li's mod is: %li\n",((curr -> base) - (curr->base)%(curr->size))/(curr->size), curr -> mod );
		if (curr -> mod == 0)
		{
			cdec = bin2dec(curr -> r_bit, 8);

			if(cdec < lowest_dec_bit1)
		 	{
		 		lowest_dec_bit1 = cdec;
		 	}
		}

		curr = curr->next;
	}
	printf("lowest_dec_bit1 is %i \n", lowest_dec_bit1);

	curr = q -> front;

	//find who has the lowest_arrival_time for mod = 0
	for (i=1;i<num_of_pages;i++)
	{
		printf("i: %i \n", i);
		if (curr -> mod == 0)
		{
			...
			if (cdec == lowest_dec_bit1)
			{
		 		if (curr -> arrivalTime < lowest_arrival_time1)
		 		{
		 			lowest_arrival_time1 = curr -> arrivalTime;
		 			//copying into non modified
		 			for(j = 0; j < 8; j++)
		 			{
		 				non_modified[j] = curr->r_bit[j];
		 			}
		 		}
			}
		}
	...
	printf("lowest_arrival_time1 is %i \n", lowest_arrival_time1);
	
	curr = q -> front;

	//find what is the lowest_dec_bit for mod = 1
	for (i=1;i<num_of_pages;i++)
	{
		// printf("Base %li's mod is: %li\n",((curr -> base) - (curr->base)%(curr->size))/(curr->size), curr -> mod );
		if (curr -> mod == 1)
		{
			...
			// printf("Page %li's cdec is: %i\n", ((curr -> base) - (curr->base)%(curr->size))/(curr->size), cdec );

			if(cdec < lowest_dec_bit2)
		 	{
		 		lowest_dec_bit2 = cdec;
		 	}
		}
		curr = curr->next;
	}

	printf("lowest_dec_bit2 is %i \n", lowest_dec_bit2);

	curr = q -> front;

	//find who has the lowest_arrival_time for mod = 1
	for (i=1;i<num_of_pages;i++)
	{
		if (curr -> mod == 1)
		{
			// printf("Page %li's arrivalTime is: %i\n", ((curr -> base) - (curr->base)%(curr->size))/(curr->size), curr -> arrivalTime );
			cdec = bin2dec(curr -> r_bit, 8);
			// printf("Page %li's cdec is: %i\n", ((curr -> base) - (curr->base)%(curr->size))/(curr->size), cdec );
			if (cdec == lowest_dec_bit2)
			{
				// printf("it happened. \n");
		 		if ((curr -> arrivalTime) < lowest_arrival_time2)
		 		{
		 			lowest_arrival_time2 = curr -> arrivalTime;
		 			//copying into non modified
		 			for(j = 0; j < 8; j++)
		 			{
		 				modified[j] = curr->r_bit[j];
		 			}
	...
	printf("lowest_arrival_time2 is %i \n", lowest_arrival_time2);
	
	curr = q -> front;

	//Ok now we compare lowest_dec_bit1 and lowest_dec_bit2



	if (pickBestReplace(modified,non_modified,8) == 1)
	{
		//choose the one have been modified
		while (curr != NULL)
		{
			// printf("Im in the loop! \n\n");
			cdec = bin2dec(curr -> r_bit, 8);
			if ((cdec == lowest_dec_bit2) && ((curr -> arrivalTime) == lowest_arrival_time2))
			{
				printf("Ok the fucking victim is %li! He/she has been modified.\n",((curr -> base) - (curr->base)%(curr->size))/(curr->size));
	...
	else if (pickBestReplace(modified,non_modified,8) == 0)
	{
		//choose the one have not been modified

		while (curr != NULL)
		{
			// printf("Im in the loop! \n\n");
			cdec = bin2dec(curr -> r_bit, 8);
			if ((cdec == lowest_dec_bit1) && ((curr -> arrivalTime) == lowest_arrival_time1))
			{
				printf("Ok the fucking victim is %li! He/she has not been modified. \n",((curr -> base) - (curr->base)%(curr->size))/(curr->size));
...
int main(int argc, char* argv[])
{
	...
	else if (strcmp(argv[4],"EARB") == 0)
	{
		...
		while (iter != NULL)	//iterate through all the instruction.
		{
			...
			//fail the above 2 case, page replacement activate.
			if (skip == 0)
			{
				printf("Page replacement active. \n");

				my_mem = iter -> memory;
				my_base = (my_mem - my_mem % my_size);
				my_lim = my_base + my_size -1;

				//since we just read from memory
				totalR++;

				pickLowestDecBit2(frames, my_base, my_lim, num_of_pages, &totalW, clock);
			...
			displayP2(frames);
			printf("\n");
			printf("Total read: %li, total write: %li \n\n", totalR, totalW);
		}
	}
