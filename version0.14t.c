...
int main(int argc, char* argv[])
{
	...
	else if (strcmp(argv[4],"ESC") == 0)
	{
		...
		while (iter != NULL)	//iterate through all the instruction.
		{
			...
			if (skip == 0)
			{
				printf("Page replacement activate (ESC) \n");
				//we just read the memory, increase totalR.
				totalR++;

				printf("skip2 before first loop : %i \n", skip2);
				//first loop, looking for (0,0)
				for (i = 0; i<num_of_pages; i++)	
				{
					printf("Go to base %li .victim -> sc = %li and victim -> mod = %li \n",((victim -> base) - (victim->base)%(victim->size))/(victim->size), victim -> sc, victim -> mod);
					if (victim -> sc == 0 && victim -> mod == 0)
					{
						...
						{
							printf("Error. The page instruction is unclear. Happen at ESC, in loop 1 \n");
						}
				...
				printf("skip2 before second loop : %i \n", skip2);
				if (skip2 == 0)
				{
					for (i = 0; i<num_of_pages; i++)
					{
						printf("Go to base %li .victim -> sc = %li and victim -> mod = %li \n",((victim -> base) - (victim->base)%(victim->size))/(victim->size), victim -> sc, victim -> mod);
						// printf("activate this loop \n");
				...
				if (skip2 == 0)
				{
					printf(" \nFail two loop, now activate the next two loop! \n");
			...
			printf("Total read is: %li \n", totalR);
			printf("Total write is: %li \n", totalW);
			skip = 0;
			skip2 = 0;
			iter = iter -> next; //move to the next instruction
			printf("Current table: \n");
