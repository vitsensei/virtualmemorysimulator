...
void pickLowestDecBit(struct pageQueue* q, int new_base, int new_limit,long int* totalWp)
{
	struct Page* curr;
	curr = q->front;
	...
	// printf("Final lowest_dec_bit is %i \n", lowest_dec_bit);
	...
	else if (strcmp(argv[4],"ARB") == 0)
	{
		...
		while (iter != NULL)	//iterate through all the instruction.
		{
			...
			if (skip == 0)
			{
				...
				pickLowestDecBit(frames, my_base, my_lim, &totalW);
